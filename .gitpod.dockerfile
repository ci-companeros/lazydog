# .gitpod.dockerfile
#
# This Dockerfile configures the development environment for your Gitpod workspace.
# It extends the Gitpod base image (`gitpod/workspace-base`) and installs additional tools
# and dependencies necessary for the project, including Node.js, Python, MongoDB, PostgreSQL,
# and Heroku CLI. This file ensures that the development environment is fully automated and 
# consistent across all users working within Gitpod.
#
# Key sections:
# 1. **Node.js**: Installs Node.js, TypeScript, and other Node utilities like Yarn and node-gyp.
# 2. **Python**: Sets up Python (version 3.12.2) with virtual environments and pip packages.
# 3. **Heroku CLI**: Installs Heroku CLI for deployment and management of Heroku applications.
# 4. **MongoDB & PostgreSQL**: Sets up MongoDB and PostgreSQL, including aliases to easily start these services.
# 5. **Custom Aliases**: Provides helpful command shortcuts like `run` for running the Django server.
#
# Author: JaqiKal
